...
function makeCtxWithHubs(): {
  ...
  // We only need stateHub and runHub on ctx for eventForwarder.
  const ctx = { stateHub, runHub } as unknown as RunnerContext;
  return { ctx, stateHub, runHub };
}
...
describe("startEventForwarding", () => {
  ...
  it("stops forwarding after disposer is called", () => {
    const { ctx, stateHub, runHub } = makeCtxWithHubs();
    const { client, sent } = makeFakeClient();
