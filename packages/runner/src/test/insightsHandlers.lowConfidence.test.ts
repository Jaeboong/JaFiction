...
import { analyzeProjectInsightsService, generateProjectInsightsService } from "../routes/insightsHandlers";
...
const jobPostingModule: {
  fetchAndExtractJobPosting: typeof import("@jasojeon/shared").fetchAndExtractJobPosting;
} = require("../../../shared/dist/core/jobPosting");
...
async function createHarness(): Promise<Harness> {
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasojeon-lowconf-"));
...
function makeExtractionResult(overrides: Partial<JobPostingExtractionResult>): JobPostingExtractionResult {
  return {
    source: "url",
    fetchedAt: "2026-04-17T00:00:00.000Z",
    normalizedText: "공고 본문",
    companyName: "테스트회사",
    roleName: "백엔드 개발자",
    keywords: [],
    warnings: [],
...
test("analyzeProjectInsightsService: both role tier → postingReviewReasons includes lowConfidenceExtraction + reviewNeeded", async (t) => {
  ...
  const originalFetch = jobPostingModule.fetchAndExtractJobPosting;
  t.after(() => {
    jobPostingModule.fetchAndExtractJobPosting = originalFetch;
  });

  jobPostingModule.fetchAndExtractJobPosting = async () =>
  ...
  await analyzeProjectInsightsService(harness.ctx, { projectSlug: project.slug });

  const refreshed = await harness.storage.getProject(project.slug);
  assert.equal(refreshed.insightStatus, "reviewNeeded");
  assert.ok(
    refreshed.postingReviewReasons.includes("lowConfidenceExtraction"),
    "should include lowConfidenceExtraction when both tiers are role"
  );
  assert.equal(
    refreshed.jobPostingFieldConfidence.companyName,
    "role",
    "jobPostingFieldConfidence should reflect extraction fieldSources"
  );
});

test("analyzeProjectInsightsService: companyName is factual → lowConfidenceExtraction not added", async (t) => {
  const harness = await createHarness();
  t.after(async () => harness.cleanup());
  ...
  assert.ok(
    !refreshed.postingReviewReasons.includes("lowConfidenceExtraction"),
    "should NOT include lowConfidenceExtraction when companyName is factual"
  );
  assert.equal(
    refreshed.jobPostingFieldConfidence.companyName,
    "factual",
    "jobPostingFieldConfidence.companyName should be factual"
  );
});

test("analyzeProjectInsightsService: missing companyName and roleName → lowConfidenceExtraction added", async (t) => {
  const harness = await createHarness();
  t.after(async () => harness.cleanup());
  ...
  assert.ok(
    refreshed.postingReviewReasons.includes("lowConfidenceExtraction"),
    "should include lowConfidenceExtraction when both companyName and roleName are empty"
  );
});

test("generateProjectInsightsService: lowConfidenceExtraction in postingReviewReasons → blocks LLM, sets reviewNeeded", async (t) => {
  const harness = await createHarness();
  t.after(async () => harness.cleanup());
  ...
  companyContextModule.collectCompanyContext = async () => {
    collectCalled = true;
  ...
  assert.equal(refreshed.insightStatus, "reviewNeeded");
  assert.ok(
    refreshed.insightLastError?.includes("신뢰도"),
    "insightLastError should describe the low confidence block"
  );
  assert.equal(collectCalled, false, "collectCompanyContext should not be called");
});

test("analyze 재호출 시 postingReviewReasons를 replace로 재평가한다", async (t) => {
  const harness = await createHarness();
  t.after(async () => harness.cleanup());

  const project = await harness.storage.createProject({
    companyName: "테스트회사",
    jobPostingUrl: "https://example.com/job/replace"
  });
  ...
  // 1차 호출: both role tier → lowConfidenceExtraction 추가
  jobPostingModule.fetchAndExtractJobPosting = async () =>
    makeExtractionResult({
      companyName: "테스트회사",
      roleName: "백엔드 개발자",
      fieldSources: { companyName: "role", roleName: "role" }
    });

  await analyzeProjectInsightsService(harness.ctx, { projectSlug: project.slug });

  const afterFirst = await harness.storage.getProject(project.slug);
  assert.ok(
    afterFirst.postingReviewReasons.includes("lowConfidenceExtraction"),
    "1차 analyze 후 lowConfidenceExtraction 포함 확인"
  );

  // 2차 호출: companyName factual tier → 이전 이유가 replace돼야 함
  jobPostingModule.fetchAndExtractJobPosting = async () =>
    makeExtractionResult({
      companyName: "테스트회사",
      roleName: "백엔드 개발자",
      fieldSources: { companyName: "factual", roleName: "role" }
    });

  await analyzeProjectInsightsService(harness.ctx, { projectSlug: afterFirst.slug });

  const afterSecond = await harness.storage.getProject(project.slug);
  assert.ok(
    !afterSecond.postingReviewReasons.includes("lowConfidenceExtraction"),
    "2차 analyze 후 lowConfidenceExtraction이 replace돼 사라져야 함"
  );
});

test("generateProjectInsightsService: empty postingReviewReasons → normal path proceeds", async (t) => {
  const harness = await createHarness();
  t.after(async () => harness.cleanup());
  ...
  companyContextModule.collectCompanyContext = async () => companyContextBundle;
