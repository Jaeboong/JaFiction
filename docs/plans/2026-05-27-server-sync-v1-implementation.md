...
> **암호화 모델 = at-rest, 서버 키 보유 (사용자 확정 2026-05-27).** 다기기 복호화는 "서버가 키를 들고 복호화→TLS로 평문 전달"로 동작한다(기기가 직접 ciphertext를 복호화하지 않음).
> **한계 명시**: 서버 프로세스는 병합을 위해 메모리에서 복호화 → 침해된 서버/운영자로부터는 보호 못 함. 보호 대상은 "저장 매체/백업 덤프". E2E(서버 무지)는 키 분배(암호문구) 문제로 v2. 옵트인 모달에 이 범위를 정확히 고지한다.
