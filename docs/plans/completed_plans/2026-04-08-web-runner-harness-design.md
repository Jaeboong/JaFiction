# JaFiction 웹/러너 하네스 설계서

> `forJob`의 저장소 하네스를 JaFiction 현재 구조에 맞게 변형하고, 기존 VSIX 설치 하네스를 웹 UI + 로컬 러너 개발 적용 하네스로 재정의하는 설계

---

## 1. 문제 정의

JaFiction는 이미 `웹 UI + localhost 러너` 구조로 옮겨왔지만, 저장소 차원의 개발 하네스는 거의 없는 상태다.
...
- JaFiction에 `forJob`식 저장소 하네스 plane을 도입하되 현재 웹/러너 구조에 맞게 변형한다
...
현재 JaFiction의 런타임 진입점은 이미 명확하다.
...
JaFiction 저장소를 아래 두 plane으로 나눈다.
...
기존 `forJob`의 `deploy-wsl-extension.sh` 역할을 JaFiction에서는 아래 흐름으로 치환한다.
...
- JaFiction에는 아직 `agents/`와 같은 별도 runtime contract surface가 없다
...
JaFiction에서 필요한 하네스는 더 이상 VS Code 확장 재설치 하네스가 아니다.
...
따라서 `forJob`에서 가져올 것은 "VSIX 설치"가 아니라 "운영 모델과 검증 제어면"이며, JaFiction에서는 이를 **웹 + 로컬 러너 개발 적용 하네스**로 변형해 도입한다.
